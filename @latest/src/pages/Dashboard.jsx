...
export function DashboardMessages(props) {
    ...
    return (
        <div className="DashboardMessages">
            <p>Welcome To Your Bitcoin Dashboard, {currentUser.email}</p>
        </div>
    )
}
export function DashboardTasks(props) {
    const tasks = [
        { id: 1, name: 'BTC/USD', rate: 107.211 },
        { id: 2, name: 'BTC/CAD', rate:97.547 },
        { id: 3, name: 'BTC/SGD', rate: 106.377 }
    ]
    return (
        <div className="DashboardTasks">
            <ul className="tasks">
                {tasks.map(task => <li key={task.id}>{task.name} Rate: {task.rate}</li>)}
            </ul>
        </div>
    )
}